...
class PixelNorm(nn.Module): # replacement for batch nomalization
    def __init__(self):
...
class Generator(nn.Module):
    def __init__(self, z_dim, in_channels, img_channels=3):
        ...
        # Passage en rgb après les blocs convolutifs, cette étape est nécessaire après chaque convBloc y compris le premier
        self.initial_rgb = WSConv2d(in_channels, img_channels, kernel_size=1, stride=1, padding=0)
        # Listes pour les blocs progressifs et les couches de passage en RGB, la seconde couche est initialisée avec une couche rgb qui correspond à celle nécessaire après le bloc initial
        self.prog_blocks, self.rgb_layers = (nn.ModuleList([]), nn.ModuleList([self.initial_rgb]))

        for i in range(len(factors) - 1):  # -1 to prevent index error because of factors[i+1]
            conv_in_c = int(in_channels * factors[i])
            conv_out_c = int(in_channels * factors[i + 1])
            # Ajout d'un bloc convolutif avec les dimensions souhaitées (répertoriées dans factors)
            self.prog_blocks.append(ConvBlock(conv_in_c, conv_out_c))
            # Ajout de la couche de passage en RGB correspondante au bloc qui vient d'être créé
